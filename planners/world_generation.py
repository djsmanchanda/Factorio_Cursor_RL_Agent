...
def _electronics_world() -> dict:
    ...
    return {
        ...
        "offshore_pump_sites": [{"position": [20.5, 83.5], "output": [20, 82], "resource": "water", "capacity_per_second": 1200.0, "direction": "south"}],
...
def default_world_payload() -> dict:
    ...
    payload = {
        ...
        "water_lake": {"x1": 14, "y1": 84, "x2": 27, "y2": 96, "tile": "water",
                       "offshore_edge_candidates": [{"position": [20.5, 83.5], "output": [20, 82], "direction": "south"}]},
...
def _validate_geometry(payload: Mapping) -> None:
    ...
    for site in lake["offshore_edge_candidates"]:
        x, y = site["position"]
        if not (site["direction"] == "south" and lake["x1"] <= x <= lake["x2"]
                and y == lake["y1"] - 0.5):
            raise ValueError("South-facing offshore pump candidate must sit on the north lake edge")
