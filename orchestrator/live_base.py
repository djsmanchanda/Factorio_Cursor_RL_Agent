...
def direct_plate_starter(
    ...
    lua = (
        ...
        "if dual then sx,sy=cp[1]-6*px,cp[2]-6*py;"
        "local sf={cp[1]-3*px,cp[2]-3*py};local si={cp[1]-px,cp[2]-py};"
...
def direct_plate_starter_site(
    ...
    *, search_radius: float = 400.0,
    ...
    lua = (
        ...
        "local sd=px==0 and (py<0 and defines.direction.north or defines.direction.south) "
        "or (px>0 and defines.direction.east or defines.direction.west);"
        "local sp=dual and {cp[1]-6*px,cp[2]-6*py} or {fp[1]-3*px,fp[2]-3*py};"
        "local extra=not two or (mines_only(sp) and can('electric-mining-drill',sp,sd,1.4) "
        "and (not dual or (can('electric-furnace',{cp[1]-3*px,cp[2]-3*py},nil,1.4) "
        "and can('fast-inserter',{cp[1]-px,cp[2]-py},nil,0.4) "
