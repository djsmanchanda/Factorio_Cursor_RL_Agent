...
- 2026-09-11: Fixed premature livelock fires during bot-built coverage waves: core-mall prerequisite coverage waits now pace like plate-starter waits so the 12-pass bound keeps its ~120s horizon (cycle 21 died mid coal wave at 128 tiles; identical Sep-10 terminal at 86 tiles). Evidence: cadence proof unifying 4 same-mechanism terminals; 22 focused loop-bound tests passed. Python only: fresh episode activates; no mod redeploy.
- 2026-09-11: Broke the steel circular gate: oil-ladder loans underway on proven seed steel now promote the 6-furnace district before advanced circuits (cycle 22 died at 100 passes with oil-refinery 0/1 on 1 transferable steel while promotion waited on circuits that wait on oil). Gated on live oil loan + started seed production + unstarted power capability; normal gate otherwise intact. Evidence: 15 focused steel-bootstrap tests passed. Python only: fresh episode activates; no mod redeploy.
- 2026-09-11: Fixed the latest pre-plastic deadlock: completed exact-target loans were excluded from the global completion sweep after their demand retired, leaving the fulfilled splitter batch on the circuit producer. The sweep now services finite completion and finished spare ceilings without unrelated blocking pressure through normal recipe/requester restoration. Preserved then replaced the uncommitted surplus-relocation workaround; no new stock-transfer capability remains. Evidence: four initially failing lifecycle cases and 268 final focused handoff/stock/loan/budget/chemical tests passed; review in docs/deterministic/plastic_blocker_review_2026-09-11.md. Python only: runner restart activates; no mod redeploy/Factorio restart. Live plastic remains unverified; prior oil retry-siting issue separately open. No live actions.
- 2026-09-11: Added opt-in plastic reliability campaign: three same-candidate fresh episodes with 120 game seconds of craft-counter/provider evidence before research promotion; pinned source input, independently reviewed and test-gated scoped fixes, persistent deadline/pending review, runtime lock, result archive, and last-known-good certificate. Opening oil plans now persist before packet release and replay exact geometry through existing checks after waits/restarts. Optional failed-world checkpoints pair named server saves with durable sidecars and hash-verified extraction; live replay requires a separate diagnostic runtime and cannot count as fresh acceptance. Evidence: full suite 2,829 cases (31 skipped; five sandbox-denied loopback cases subsequently passed), plus 103 final focused reliability/oil/acceptance tests; shell syntax/diff checks passed. Workflow: docs/deterministic/reliability_workflow.md. No live campaign, save, mod deployment or server action performed. Restart controller/runner for activation; live plastic and checkpoint capture remain unverified.

- 2026-09-11: Added console-controlled 12-hour OpenCode reliability supervision with four persistent Muse Spark 1.3 Contributor/xhigh aspect sessions, shared episode findings, terminal cross-review, a separate fixer and read-only independent reviewer. The secondary helper is unchanged. Exact fresh-lifecycle intent, active episode, deadline and interrupted-fix baseline survive controller failure; bounded recovery never blindly resets. Declined partial edits resume investigation, and commits reject undeclared new changes. Console displays phases, role status, milestone timings and plastic acceptance, and blocks competing lifecycle/research actions while owned. Per-fix verification runs the non-slow/non-exhaustive suite plus changed regression files unfiltered. Evidence: 2,872 full-suite cases (31 skipped; five sandbox-denied local-socket tests passed on permitted rerun), 96 integration-focused checks, and 27 final controller/reliability checks; JavaScript syntax, diff checks and fixture-only browser preview passed. Restart Operations Console to expose the controls, then explicitly start a window. No live campaign, model inference, Factorio lifecycle or mod deployment performed; sustained live plastic remains unverified. See docs/deterministic/reliability_workflow.md.
- 2026-09-11: Fixed the +3447s coal belt_bridge_unroutable terminal (episode-20260911T151719Z-18290, bridge (-324.5,12.5)->(-261.5,-16.5) entry=east exit=west destination=west). Direct through-belt taps now walk the live straight run to its downstream head instead of one tile east upstream, so the forced tile lands on free ground beyond the line while preserving the observed heading; ghosts and non-live callers keep the legacy offset. Evidence: legacy-tap reproduction of the exact terminal signature vs head-tap routing (100-action transport-belt plan); 30 belt-feed, 27 survey/tier, and 111 bridge/chemical/oil/transport tests passed; compile and diff checks clean. Python only: restart runner to activate; no mod redeploy or Factorio restart required. Live rerun past the coal bridge remains unverified; campaign and failed world untouched.

- 2026-09-12: Fixed episode-30357 circuit starvation: unchanged optional core-mall promotion waits consumed every pass, leaving the circuit loan on its capped cable prerequisite. Shared deferral handling now yields unchanged supply waits to queued work; successful loan configuration/restoration/promotion forces re-observation without claiming production progress, and coverage pacing remains intact. Consolidated observer/controller response parsing in tools/campaign_protocol.py; tool-only transport cannot authorize a fix/review, and statuses are role-specific. Evidence: 437 focused tests passed, including a reproduced cable-to-circuit scheduling failure, real promotion/restoration boundaries and transport verdict tests; diff check clean. Diagnosis: docs/deterministic/circuit_starvation_review_2026-09-12.md. Python only: restart affected runner/controller. No live reset, deployment or factory change performed; plastic still needs fresh acceptance. Latest failed run was research on be7c7be, not the supervised plastic acceptance workflow.

- 2026-09-12: Added bounded oil-route strategy escalation (local land, underground bypass, wider land, water) with exhaustion reasons and unchanged legality/purity checks. Refactored fluid planning to return placements and segments from one search, removing duplicate successful routing work. Targeted layout audit records fixed mall/chemical templates and remaining absolute starter-anchor defaults; no claim of coordinate-free planning. Evidence: 82 fluid/chemical/transaction checks passed; audit at docs/deterministic/layout_recovery_audit_2026-09-12.md. Python only; restart affected runner/controller. No live operations or measured runtime-speed claim.

- 2026-09-12: Enforced blueprint-only player-force deterministic construction without bootstrap gifts. Python converts every legacy direct placement to a funded ghost, reconciles existing targets as configuration-only, and applies chest settings only after bots build. Lua rejects missing direct targets before mutation and prevents post-removal direct creation; player requester destroy/recreate trash workaround is disabled. Both profiles grant no items; supply helper validates rather than inserts. Evidence: 381 focused Python/Lua checks passed, including item-class coverage, post-build configuration ordering, and independent Lua rejection; diff clean. Requires mod synchronization plus Factorio and Python consumer restarts; no deployment or live mutation performed. Cold-start requester shortages are now real supply blockers, never subsidized.

- 2026-09-12: Mall AM2 promotion now begins after the first complete iron foundation has measured replacement output, ahead of later foundations and full core-mall readiness. Queues missing AM2 replacements even before their producer exists; native bot orders require proven production and unreserved transferable surplus. Fast-inserter upgrades keep their later gate. Evidence: 38 tier/bootstrap/controller-bound tests passed. Python runner restart required; no live action.

- 2026-09-12: Limited each mall native-upgrade submission to one machine, even with enough stock for the entire row; preserves construction reserves. Tier/stock checks passed. Python runner restart activates.

- 2026-09-12: Fixed episode-23634 steel/pole deadlock: compact steel now claims its startup reserve under the same conversion_steel-plate material project and priority. Active legacy split reservations converge without taking unrelated bridge stock; restart preserves expanded bills and constructing state. Nested shortage recovery now queues one attempt rather than escaping as an unhandled exception; hard failures and chemical handoffs still propagate. Luna high implementation and Astra high independent review completed. Evidence: 342 focused checks passed, including real ledger/steel-plan replay with four poles, two retained for the bridge, reload/idempotence, restart states and bounded recovery. Python only: restart runner/controller; no Lua deployment or Factorio restart required. No live actions performed; steel/plastic production remains unverified on this candidate.
- 2026-09-12: Bounded `DEFERRED CONTROL` logging as factory state grows. Repeated waits now show origin counts/states, at most four active project IDs with the blocked-item quantity, the material-ledger revision, and tracked-stock deltas instead of copying every project bill and full inventory snapshot. Full bills remain in the episode material ledger. Evidence: 23 focused loop-bound tests passed. Python only: restart the affected runner/controller; no mod redeploy or Factorio restart.
- 2026-09-12: Fixed episode-32139's false `no_progress` stop during the remote coal coverage corridor. The controller now credits strictly closer observed built-roboport frontiers, so completing one hop and submitting its replacement ghost resets the unchanged-pass guard even when total ghosts stay flat; unchanged or retreating frontiers still receive no credit. Evidence: 180 focused coverage/controller/chemical tests passed; the 822-test fast selection completed with 821 passed and one skipped; compilation and diff checks passed. Python only: restart the affected runner/controller; no mod redeploy or Factorio restart.
- 2026-09-13: Simplified the supervised OpenCode loop: GLM-5.3 auto owns controller/fixer work, GLM-5.3 high reviews only changed candidates, four Muse Spark high observers remain, and the first two observers opportunistically use Zen Muse Spark with Go fallback. Candidate verification is targeted to changed test files instead of a repository-wide sweep. Python-only; no Lua deployment or Factorio restart.
- 2026-09-14: Capped the shared mall at 8 assembler slots until advanced-circuit production has actually started; it then expands to the existing 12-slot demand bank. Added transition tests for both sides of the gate. Python-only; restart the runner/controller before the next campaign.
- 2026-09-14: Fixed persistent-intermediate starvation exposed by the 8-slot cap. Borrowing a cell now invalidates stale managed-source pointers; stopped assembler footprints no longer count as replenishing producers; persistent intermediate readiness uses transferable stock rather than aggregate force stock. Evidence: 184 focused construction/bootstrap/intermediate/stock tests passed. Python-only; restart the runner/controller before the next campaign.
- 2026-09-14: Goal-critical requester/passive-provider chest demand now yields optional core-mall promotion, preserving the full eight-slot pre-advanced-circuit pool for the automation-science chemical ladder instead of looping on fast-inserter promotion. Evidence: 174 focused construction/intermediate/stock tests passed. Python-only; restart the runner/controller before the next campaign.
- 2026-09-14: Changed mall policy so iron-stick and pole batches rotate through the shared pre-advanced-circuit pool; permanent independent mall cells unlock when advanced-circuit production starts. Persistent iron-stick sourcing resumes only after that transition. Python-only; restart the runner/controller before the next campaign.
- 2026-09-14: Removed the post-advanced-circuit mall capacity ceiling entirely; after the transition, cells are created as needed and only the pre-advanced-circuit 8-slot guard remains. Evidence: 243 focused mall/reservation/recovery/bootstrap tests passed. Python-only; restart the runner/controller before the next campaign.

- 2026-09-14: Added generic missing-producer recovery, durable nonblocking stock waits and cycle evidence, project-derived belt commitments, optional transport-electricity decisions, bounded live extraction-route preflight, mandatory campaign checks and provenance-verified diagnostic replay pairs, and safe-partial RL learning with scalable mining candidates/V3 observations. Retained the eight-slot unknown-future guard and unchanged seed supply; no speculative capability fence. Conditional funded bot-built steam remains bounded and offline-validated. Fixed the first user-started candidate run: Factorio J/tick values now convert with 60/1000, steam uses measured one-minute consumption, and coal construction shortages queue instead of escaping. Read-only preserved-episode verification reports 10000 kW firm, about 242 kW consumption and zero requested steam units. Evidence: 828-case fast gate (one skip), 95 final integration-focused cases, 62 training cases and 33 HTTP/observer cases passed; broader 2907-case run exposed two steel harness regressions, corrected and rechecked. Luna/Terra high implementation and Luna high reviews; no save/reset/deployment. Restart affected Python runner/controller; no Lua mod redeploy or Factorio restart required. Full fresh-run acceptance, live steam construction/retirement, complete future chemical-bill admission and reduced-seed certification remain unproven.
- 2026-09-14: Fully occupied power-hookup areas now yield a typed `power_bridge_anchor_wait` with candidate counts and sampled occupying entities instead of aborting as `untyped_stuck`; focused infrastructure, coverage, ghost, mining, chemical, and smelter checks passed (307 cases). Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Fully occupied inline direct-belt taps now yield `direct_belt_approach_wait` with feed direction, blocked approach tiles, and sampled occupying entities instead of aborting as `untyped_stuck`; belt, route, occupancy, and intermediate-promotion checks passed (134 cases). Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Periodic completed-loan release now catches typed chemical-ladder handoff waits after restoring a borrowed cell, emits a retry-after-reobservation message, and keeps the controller alive. Evidence: 368 focused construction, reservation, conversion, run-loop, prep, and chemical checks passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Native mall upgrades now exclude active bootstrap-loan machine positions, preventing a live temporary pipe cell from being upgraded during its loan. Periodic loan release also parks `configure_target_missing` identity races for re-observation. Evidence: 246 focused tier, handoff, construction-stock, and reservation checks passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Added the reduced-seed quad mall module: after the iron pioneer lifecycle reaches `released`, the pre-advanced pool expands from 8 to 12 assembler slots by adding four machines around one requester and two output providers. Existing paired cells remain the fallback before that gate. Evidence: 190 focused quad, mall-density, construction-stock, recovery, and baseline checks passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Fixed the pre-quad AM1 starvation terminal: AM1 is now a just-in-time evergreen reserve (target 6, floor 3) backed by the existing rotating loan path, and reserve-backed draws no longer poison the unbacked-draw verdict. The watchdog only activates after an AM1 starter has actually been observed, so cold-start harnesses and the belt reserve remain unchanged. Evidence: 275 focused mall/bootstrap/construction/reservation tests passed, including a reproduced native-upgrade drain and AM1 requeue. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Corrected the quad expansion bay geometry: its former `(start+4,start+24)` center overlapped legacy paired row four, so the denser module could not be placed. The quad now uses a reserved bay to the right of the paired bank, with a regression check proving the full staged footprint clears the legacy service area. Evidence: 221 focused quad/mall-density/construction/reservation tests passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Quad mall material reservations now use the quad topology's complete bill, including support inserters, before submission. The live run had selected `(78.5,37.5)` but discovered long-handed/fast-inserter shortages only during staging, leaving the old bank visible while the new module waited. Evidence: 222 focused quad/mall-density/construction/reservation tests passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Starter-backed live runs now begin with the dense four-machine mall module instead of the legacy paired bank; after the iron pioneer releases, two reserved staggered modules expand it to twelve slots. Module centres and support substations remain outside the legacy bank footprint, while dry harnesses retain the old paired contract. Evidence: 279 focused mall/bootstrap/construction/reservation tests passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Broke the first dense-module bootstrap cycle: its seed half now uses stocked regular inserters, avoiding the unavailable fast/long-handed inserters that previously left the first mall with no borrowable assembler and terminated at `rationed_mall_no_borrower`. Later dense modules keep the full support-inserter bill. Evidence: 279 focused tests passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Refined the mall progression to two regular-inserter paired cells first (four slots), then top/bottom dense expansion only after long-handed-inserter production or stock is observed. Expansion keeps regular inserters for I/O initially, allowing later AM1→AM2 and inserter→fast upgrades without making fast inserters a placement prerequisite. Evidence: 279 focused mall/bootstrap/construction/reservation tests passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-15: Added evergreen floors for electronic circuits (18 target, 6 floor) and regular inserters (20 target, 5 floor). The two-pair seed had correctly built its circuit cell, then its stock gate stayed disabled while fast-inserter, drill, and AM1 work consumed the reserve; the floors now re-queue those batches before prerequisites hit zero. Evidence: 279 focused tests passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-16: Made seed-reserve observation durable even when the survey receives an immutable dry `prepped` set. Electronic-circuit and inserter stock that was observed before a drain now re-queues after the live inventory omits the zero-count item, preventing the repeated fast-inserter/electronic-circuit cycle seen in episode-24669. Evidence: 275 focused tests passed. Python only; restart the runner/controller before the next campaign.
- 2026-09-16: Added the isolated eight-lane deterministic checkpoint fleet: immutable/versioned C0-C6 bundles, structured live milestone evidence, five-minute/milestone/terminal capture, 20-per-origin retention, pinned commit/generation inputs, multi-commit scheduling, matched promotion verification, 2.5x timing warnings, stability-first ranking, lane-selected helpers, persistent service control, and responsive Operations Console matrix/controls. Evidence: 83 focused fleet checks and 82 affected runtime/dashboard checks passed; the 3,080-case suite had five sandbox-only HTTP failures that passed with permission and two order-sensitive material tests that passed individually and as their 69-test file; compile, shell/JS syntax, diff, and 1120px visual layout checks passed. Python/UI only: restart Operations Console and affected runners; initialize/install/start the fleet service explicitly for live use. No service, server, save reset, mod deployment, or live Factorio action was performed; live capture/reload and eight-server acceptance remain unverified.
