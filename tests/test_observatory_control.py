...
def test_training_start_request_validates_bounded_runtime_settings() -> None:
    request = TrainingStartRequest.from_payload({
        "server_count": 4, "runner_limit": 16, "initial_runners": 8, "start_seed": 1200,
    })
...
@pytest.mark.parametrize("field,value", [
    ...
    ("initial_runners", 17), ("start_seed", -1),
])
def test_training_start_request_rejects_out_of_bounds_values(field: str, value: int) -> None:
    payload = {"server_count": 4, "runner_limit": 16, "initial_runners": 8, "start_seed": 1200}
    payload[field] = value
...
def test_supervisor_starts_configured_workers_and_controller(monkeypatch, tmp_path) -> None:
    ...
    request = TrainingStartRequest(2, 12, 4, 99)
    ...
    monkeypatch.setattr(observatory_control.subprocess, "Popen", lambda *args, **kwargs: fake_process)
    ...
    try:
        start = Request(
            f"{base}/api/training/start", method="POST", data=json.dumps({
                "server_count": 2, "runner_limit": 12, "initial_runners": 4, "start_seed": 99,
            }).encode("utf-8"), headers={"Content-Type": "application/json"},
        )
        with urlopen(start, timeout=2) as response:
            assert response.status == 202
        assert control.started == TrainingStartRequest(2, 12, 4, 99)
